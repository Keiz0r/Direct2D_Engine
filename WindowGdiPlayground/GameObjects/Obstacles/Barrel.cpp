...
Barrel::Barrel(const D2D1_POINT_2F& position, const float& opacity, const float& rotationAngle, const bool& is_destrucable)
	:
...
void Barrel::draw() {
	if (s_pSprite != nullptr) {
		...
		float radiusFromScreenCenter = (position.x - centerCoordinates->x) * (position.x - centerCoordinates->x) + (position.y - centerCoordinates->y) * (position.y - centerCoordinates->y);

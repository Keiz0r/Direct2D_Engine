	...
	if (s_pSprite != nullptr) {
		...
		if (m_blinkframes == 0) {
			if (visibilityRadius > radiusFromScreenCenter) {
				s_pgfx->transformTRSM(0.0f, LegOffset * m_fScalar, m_rotationAngle, { shift.x, shift.y }, m_fScalar, m_fScalar, false);
				s_pgfx->drawBitmap(s_pSprite, { shift.x - halfwidth, shift.y - halfheight, shift.x + halfwidth, shift.y + halfheight }, m_opacity, { 24.0f, 26.0f, 43.0f, 47.0f });
